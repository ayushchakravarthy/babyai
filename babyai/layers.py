...
class ConvolutionalNet(nn.Module):
    """Simple conv. net convolves the input channel but retains input image width."""
    def __init__(
        self,
        num_channels: int,
        cnn_kernel_size: int, 
        ...
        self.conv_1a = nn.Conv2d(in_channels=num_channels, out_channels=num_conv_channels, kernel_size=(3, 3),
                                padding=1, stride=stride)
        self.bn1a = nn.BatchNorm2d(num_conv_channels)
        self.conv_1b = nn.Conv2d(in_channels=num_channels, out_channels=num_conv_channels, kernel_size=(3, 3),
                                padding=1, stride=stride)
        self.bn1b = nn.BatchNorm2d(num_conv_channels)

        self.conv_2a = nn.Conv2d(in_channels=num_channels, out_channels=num_conv_channels, kernel_size=5,
                                stride=stride, padding=2)
        self.bn2a = nn.BatchNorm2d(num_conv_channels)
        self.conv_2b = nn.Conv2d(in_channels=num_channels, out_channels=num_conv_channels, kernel_size=5,
                                stride=stride, padding=2)
        self.bn2b = nn.BatchNorm2d(num_conv_channels)
        ...
        self.bn3 = nn.BatchNorm2d(num_conv_channels)

        self.dropout = nn.Dropout(p=dropout_probability)
        self.relu = nn.ReLU()
        layers = [self.relu, self.dropout]
        self.layers = nn.Sequential(*layers)
        self.output_dimension = num_conv_channels * 5

    def forward(self, input_images: torch.Tensor) -> torch.Tensor:
        """
        :param input_images: [batch_size, image_channels, image_width, image_width]
        :return: [batch_size, image_width * image_width, 5 * num_conv_channels]
        """
        batch_size = input_images.size(0)

        conved_1 = self.conv_1a(input_images)
        conved_1 = self.bn1a(conved_1)

        conved_2 = self.conv_1b(input_images)
        conved_2 = self.bn1b(conved_2)

        conved_3 = self.conv_2a(input_images)
        conved_3 = self.bn2a(conved_3)

        conved_4 = self.conv_2b(input_images)
        conved_4 = self.bn2b(conved_4)

        conved_5 = self.conv_3(input_images)
        conved_5 = self.bn3(conved_5)

        images_features = torch.cat([conved_1, conved_2, conved_3, conved_4, conved_5], dim=1)
